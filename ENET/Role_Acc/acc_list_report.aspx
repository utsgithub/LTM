...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <asp:SqlDataSource ID="SqlDataSource5" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>"
        SelectCommand="SELECT Districts as 'District', MAX(datename(month, iDate)) as 'Month', SUM(labour) as 'Total Labour Hours', SUM(cost) as 'Total Labour Cost' FROM view_detail_interventions WHERE status='completed' GROUP BY Districts, DATEPART(month, iDate) ORDER BY DATEPART(month, iDate) ASC"></asp:SqlDataSource>
    ...
    <asp:GridView ID="ReportGridView5" runat="server" AutoGenerateColumns="true" DataSourceID="SqlDataSource5"
        GridLines="None" CssClass="table table-bordered table-striped table-hover" />
